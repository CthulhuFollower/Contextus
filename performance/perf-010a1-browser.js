...
function runApplicationFrame(dbName, runId, mode) {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      frame.remove();
      reject(new Error(`Timeout esperando PERF-010A1: ${runId}`));
    }, 120_000);
    ...
    const query = new URLSearchParams({
      ...
      perfRenderCulling: mode === "culled" ? "1" : "0",
...
async function run() {
  ...
  const sizes = parseList("sizes", [1_000, 10_000, 50_000], Number).filter(Number.isFinite);
  const cameras = parseList("cameras", PERF_010A1_CAMERAS);
  const modes = parseList("modes", PERF_010A1_MODES);
  const results = [];
    ...
    for (const cameraName of cameras) {
      ...
      for (const mode of modes) {
        ...
        for (let index = 0; index < sampleCount(nodeCount); index += 1) {
          ...
          const dbName = `contextus-perf-010a1-${nodeCount}-${cameraName}-${mode}-${index}`;
          await seedDatabase(dbName, fixture);
          profiles.push(await runApplicationFrame(dbName, `${dbName}-${Date.now()}`, mode));
        ...
        results.push({
          ...
          mode,
          profiles,
          summary: summarizePerf010A1Profiles(profiles)
        });
