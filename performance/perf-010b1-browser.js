...
import {
  PERF_010B1_MODES,
  summarizePerf010B1Profiles
} from "./perf-010b1-core.js";
...
function runApplicationFrame(dbName, runId, mode) {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      frame.remove();
      reject(new Error(`Timeout esperando PERF-010B1: ${runId}`));
    }, 120_000);
    ...
    const query = new URLSearchParams({
      ...
      perfRenderCulling: "1",
      perfLinkSegmentCulling: mode === "b1" ? "1" : "0",
...
async function run() {
  ...
  const modes = parseList("modes", PERF_010B1_MODES);
  const results = [];
    ...
    for (const cameraName of cameras) {
      ...
      for (const mode of modes) {
        ...
        for (let index = 0; index < sampleCount(nodeCount); index += 1) {
          ...
          const dbName = `contextus-perf-010b1-${nodeCount}-${cameraName}-${mode}-${index}`;
        ...
        results.push({
          ...
          mode,
          profiles,
          summary: summarizePerf010B1Profiles(profiles)
        });
